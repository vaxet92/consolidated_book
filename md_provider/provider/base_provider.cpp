...
BaseProvider::BaseProvider(const ProviderConfig& config)
    : config(config),
      running(false),
...
void BaseProvider::Run() {
    while (running) {
        try {
            ...
            std::cout << "[" << config.exchange_name << "] Connecting to " 
                      << GetHost() << "..." << std::endl;
...
void BaseProvider::HandleReconnection() {
    if (!running) return;
    ...
    uint64_t delay_ms = std::min(
        INITIAL_RECONNECT_DELAY_MS * (1ULL << (reconnect_count - 1)),
        MAX_RECONNECT_DELAY_MS
    );
    
    std::cout << "[" << config.exchange_name << "] Reconnecting in " 
              << delay_ms << "ms (attempt " << reconnect_count << ")" << std::endl;
    
    std::this_thread::sleep_for(std::chrono::milliseconds(delay_ms));
}

void BaseProvider::PushTrade(Trade&& trade) {
...
int64_t BaseProvider::GetCurrentTimeMs() {
